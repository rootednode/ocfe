...
const Form = (params: { id: string; formid: string }): React.ReactElement => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredData = data.filter((item: any) => item.id == params.formid);
      ...
      {loading ? (
        ...
        <div className="sjs-results-content" ref={visContainerRef}>
          <div className="sjs-results-placeholder">
            <span>This form has not been filled out yet</span>
          </div>
        </div>
      )}
    </>
